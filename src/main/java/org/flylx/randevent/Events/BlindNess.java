...
public class BlindNess extends BaseRandEvent {
    Plugin plugin;
    public BlindNess(Plugin plugin, World world){
        this.plugin = plugin;
        setWorld(world);
        setEventType(RandEvents.BLINDNESS);
    }

    public BukkitTask startBlindNess(){
        ...
        setBukkitTask(Bukkit.getScheduler().runTaskTimer(plugin, () -> {
            ...
            for(Player player:getWorld().getPlayers()){
                player.addPotionEffect(PotionEffectType.BLINDNESS.createEffect(300,0));
            }
            if (timer.get() >= 24000) {
